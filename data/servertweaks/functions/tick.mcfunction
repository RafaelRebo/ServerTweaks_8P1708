...
execute as @e[tag=injection] at @e[tag=injection] unless entity @e[type=area_effect_cloud,distance=..7] run summon area_effect_cloud ~ ~0.5 ~ {Duration:2000000,CustomName:'"Click derecho para infectar"',CustomNameVisible:1b,Tags:["injection"]}
...
execute as @a[scores={hasInjection=1.., talkvill=1..},tag=vampirable] at @a run say ha sido infectado!
